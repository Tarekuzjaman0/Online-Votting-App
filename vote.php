...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($_POST['action'] === 'comment') {
        ...
        try {
            // Check if user already commented on this poll
            $stmt = $conn->prepare("SELECT id FROM comments WHERE poll_id = ? AND user_id = ?");
            $stmt->execute([$poll_id, $_SESSION['user_id']]);
            if ($stmt->fetch()) {
                ...
                header("Location: vote.php?id=" . $poll_id . "&success=comment");
                exit();
            }
        } catch (Exception $e) {
